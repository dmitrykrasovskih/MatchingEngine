...
fun mapOrdersToOrderBookPersistenceDataList(
    ...
    orderBooksSides: Collection<OrderBookSide>,
    log: Logger
): List<OrderBookPersistenceData> {
    ...
    mutableOrderBooksSides.forEach { orderBooksSide ->
        ...
        orderBookPersistenceDataList.add(
            OrderBookPersistenceData(
                orderBooksSide.assetPairId,
                orderBooksSide.isBuySide,
                emptyList()
            )
        )
    }
...
fun mapOrdersToOrderBookPersistenceDataList(orders: Collection<LimitOrder>, log: Logger) =
    mapOrdersToOrderBookPersistenceDataList(orders, emptyList(), log)
