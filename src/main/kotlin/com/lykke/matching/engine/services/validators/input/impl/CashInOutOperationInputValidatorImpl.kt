...
@Component
class CashInOutOperationInputValidatorImpl constructor(private val applicationSettingsHolder: ApplicationSettingsHolder) :
    ...
    override fun performValidation(cashInOutParsedData: CashInOutParsedData) {
        val cashInOutContext =
            (cashInOutParsedData.messageWrapper as CashInOutOperationMessageWrapper).context as CashInOutContext
    ...
    private fun isAssetExist(cashInOutContext: CashInOutContext, inputAssetId: String) {
        if (cashInOutContext.cashInOutOperation.asset == null) {
            LOGGER.info(
                "Asset with id: $inputAssetId does not exist, cash in/out operation; ${cashInOutContext.cashInOutOperation.externalId}), " +
    ...
    private fun isVolumeAccuracyValid(cashInOutContext: CashInOutContext) {
        ...
        val volumeValid = NumberUtils.isScaleSmallerOrEqual(
            amount,
            asset!!.accuracy
        )
        ...
        if (!volumeValid) {
            LOGGER.info(
                "Volume accuracy is invalid client: ${cashInOutContext.cashInOutOperation.clientId}, " +
    ...
    private fun isAssetEnabled(cashInOutContext: CashInOutContext) {
        ...
        if (amount < BigDecimal.ZERO && applicationSettingsHolder.isAssetDisabled(asset!!.symbol)) {
            LOGGER.info(
                "Cash out operation (${cashInOutContext.cashInOutOperation.externalId}) for client ${cashInOutContext.cashInOutOperation.clientId} " +
                        "asset ${asset.symbol}, " +
