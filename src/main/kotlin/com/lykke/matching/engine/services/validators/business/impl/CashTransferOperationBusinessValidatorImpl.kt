...
@Component
class CashTransferOperationBusinessValidatorImpl(private val balancesHolder: BalancesHolder) :
    CashTransferOperationBusinessValidator {
    companion object {
        private val LOGGER = LogManager.getLogger(CashTransferOperationBusinessValidatorImpl::class.java.name)
    ...
    private fun validateOverdraftLimitPositive(cashTransferContext: CashTransferContext) {
        ...
        if (overdraftLimit != null && overdraftLimit.signum() == -1) {
            throw ValidationException(
                ValidationException.Validation.NEGATIVE_OVERDRAFT_LIMIT,
                "ClientId:${transferOperation.fromClientId}, " +
    ...
    private fun validateBalanceValid(cashTransferContext: CashTransferContext) {
        ...
        val overdraftLimit =
            if (transferOperation.overdraftLimit != null) -transferOperation.overdraftLimit else BigDecimal.ZERO
        if (balanceOfFromClient - reservedBalanceOfFromClient - transferOperation.volume < overdraftLimit) {
            LOGGER.info(
                "Cash transfer operation (${transferOperation.externalId}) from client ${transferOperation.fromClientId} " +
            ...
            throw ValidationException(
                ValidationException.Validation.LOW_BALANCE, "ClientId:${transferOperation.fromClientId}, " +
                        "asset:${transferOperation.asset}, volume:${transferOperation.volume}"
            )
        }
    }
