...
class AssetOrderBook(
    brokerId: String,
    assetId: String
) : AbstractAssetOrderBook(brokerId, assetId) {
    ...
    fun setOrderBook(isBuySide: Boolean, queue: PriorityBlockingQueue<LimitOrder>) =
        if (isBuySide) bidOrderBook = queue else askOrderBook = queue
