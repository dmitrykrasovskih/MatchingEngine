...
private fun convertLimitOrder(limitOrderWithTrades: LimitOrderWithTrades): Order {
    ...
    val feeInstructions =
        if (limitOrderWithTrades.trades.isEmpty()) limitOrder.fees else limitOrderWithTrades.trades.first().fees.map { it.instruction }
    return Order(
...
private fun convertMarketOrder(marketOrderWithTrades: MarketOrderWithTrades): Order {
    ...
    val feeInstructions =
        if (marketOrderWithTrades.trades.isEmpty()) marketOrder.fees else marketOrderWithTrades.trades.first().fees.map { it.instruction }
    return Order(
        OrderType.MARKET,
        marketOrder.id,
...
private fun orderStatusInfo(order: com.lykke.matching.engine.daos.Order): OrderStatusInfo {
    return when (order.status) {
        ...
        com.lykke.matching.engine.order.OrderStatus.NotEnoughFunds.name -> OrderStatusInfo(
            OrderStatus.REJECTED,
            OrderRejectReason.NOT_ENOUGH_FUNDS
        )
        com.lykke.matching.engine.order.OrderStatus.ReservedVolumeGreaterThanBalance.name -> OrderStatusInfo(
            OrderStatus.REJECTED,
            OrderRejectReason.RESERVED_VOLUME_GREATER_THAN_BALANCE
        )
        com.lykke.matching.engine.order.OrderStatus.NoLiquidity.name -> OrderStatusInfo(
            OrderStatus.REJECTED,
            OrderRejectReason.NO_LIQUIDITY
        )
        com.lykke.matching.engine.order.OrderStatus.UnknownAsset.name -> OrderStatusInfo(
            OrderStatus.REJECTED,
            OrderRejectReason.UNKNOWN_ASSET
        )
        com.lykke.matching.engine.order.OrderStatus.DisabledAsset.name -> OrderStatusInfo(
            OrderStatus.REJECTED,
            OrderRejectReason.DISABLED_ASSET
        )
        com.lykke.matching.engine.order.OrderStatus.LeadToNegativeSpread.name -> OrderStatusInfo(
            OrderStatus.REJECTED,
            OrderRejectReason.LEAD_TO_NEGATIVE_SPREAD
        )
        com.lykke.matching.engine.order.OrderStatus.InvalidFee.name -> OrderStatusInfo(
            OrderStatus.REJECTED,
            OrderRejectReason.INVALID_FEE
        )
        com.lykke.matching.engine.order.OrderStatus.TooSmallVolume.name -> OrderStatusInfo(
            OrderStatus.REJECTED,
            OrderRejectReason.TOO_SMALL_VOLUME
        )
        com.lykke.matching.engine.order.OrderStatus.InvalidPrice.name -> OrderStatusInfo(
            OrderStatus.REJECTED,
            OrderRejectReason.INVALID_PRICE
        )
        com.lykke.matching.engine.order.OrderStatus.NotFoundPrevious.name -> OrderStatusInfo(
            OrderStatus.REJECTED,
            OrderRejectReason.NOT_FOUND_PREVIOUS
        )
        com.lykke.matching.engine.order.OrderStatus.InvalidPriceAccuracy.name -> OrderStatusInfo(
            OrderStatus.REJECTED,
            OrderRejectReason.INVALID_PRICE_ACCURACY
        )
        com.lykke.matching.engine.order.OrderStatus.InvalidVolumeAccuracy.name -> OrderStatusInfo(
            OrderStatus.REJECTED,
            OrderRejectReason.INVALID_VOLUME_ACCURACY
        )
        com.lykke.matching.engine.order.OrderStatus.InvalidVolume.name -> OrderStatusInfo(
            OrderStatus.REJECTED,
            OrderRejectReason.INVALID_VOLUME
        )
        com.lykke.matching.engine.order.OrderStatus.InvalidValue.name -> OrderStatusInfo(
            OrderStatus.REJECTED,
            OrderRejectReason.INVALID_VALUE
        )
        com.lykke.matching.engine.order.OrderStatus.TooHighPriceDeviation.name -> OrderStatusInfo(
            OrderStatus.REJECTED,
            OrderRejectReason.TOO_HIGH_PRICE_DEVIATION
        )
...
private fun convertFeeInstruction(
    index: Int,
    internalFeeInstruction: com.lykke.matching.engine.daos.v2.FeeInstruction
): FeeInstruction {
    val assetIds = (internalFeeInstruction as? NewFeeInstruction)?.assetIds
    return FeeInstruction(
        convertFeeType(internalFeeInstruction.type),
...
private fun convertFeeTransfer(
    index: Int,
    internalFeeTransfer: com.lykke.matching.engine.daos.FeeTransfer?
): FeeTransfer? {
