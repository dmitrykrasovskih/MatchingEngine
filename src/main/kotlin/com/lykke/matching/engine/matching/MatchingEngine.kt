...
class MatchingEngine(
    ...
    fun match(
        ...
        if (order.takePrice() != null && !checkExecutionPriceDeviation(
                order.isBuySide(),
                order.takePrice()!!,
                bestPrice,
        ...
        if (checkOrderBook(order, workingOrderBook)) {
            while (getMarketBalance(availableBalances, order, asset) >= BigDecimal.ZERO
                ...
                var marketRoundedVolume =
                    NumberUtils.setScale(if (isBuy) volume else -volume, baseAsset.accuracy, !isBuy)
                var oppositeRoundedVolume = NumberUtils.setScale(
                    if (isBuy) -limitOrder.price * volume else limitOrder.price * volume,
                    quotingAsset.accuracy,
                    isBuy
                )

                executionContext.info(
                    "Matching with limit order ${limitOrder.externalId}, client ${limitOrder.clientId}, price ${limitOrder.price}, " +
                ...
                if ((!order.isStraight()) && isFullyMatched) {
                    oppositeRoundedVolume = BigDecimal.valueOf(
                        order.volume.signum().toLong()
                    ) * (NumberUtils.setScale(
                        order.volume.abs() - totalLimitVolume.abs(),
                        quotingAsset.accuracy,
                        isBuy
                    ))
                    marketRoundedVolume = NumberUtils.setScale(
                        NumberUtils.divideWithMaxScale(-oppositeRoundedVolume, limitOrder.price),
                        baseAsset.accuracy,
                        !isBuy
                    )
                    executionContext.info(
                        "Rounding last matched limit order trade: ${
                ...
                val limitOrderInfo =
                    "id: ${limitOrder.externalId}, client: ${limitOrder.clientId}, asset: ${limitOrder.assetPairId}"
                ...
                if (NumberUtils.equalsIgnoreScale(
                        BigDecimal.ZERO,
                        if (isBuy) marketRoundedVolume else oppositeRoundedVolume
                    )
                ) {
                ...
                val baseAssetOperation = WalletOperation(
                    order.brokerId,
                    order.accountId,
                    order.clientId,
                ...
                val quotingAssetOperation = WalletOperation(
                    order.brokerId,
                    order.accountId,
                    order.clientId,
                    assetPair.quotingAssetId,
                    oppositeRoundedVolume,
                    BigDecimal.ZERO
                )
                val limitBaseAssetOperation = WalletOperation(
                    limitOrder.brokerId,
                    limitOrder.accountId,
                    limitOrder.clientId,
                    assetPair.baseAssetId,
                    -marketRoundedVolume,
                    if (-marketRoundedVolume < BigDecimal.ZERO) -marketRoundedVolume else BigDecimal.ZERO
                )
                val limitQuotingAssetOperation = WalletOperation(
                    limitOrder.brokerId,
                    limitOrder.accountId,
                    limitOrder.clientId,
                    assetPair.quotingAssetId,
                    -oppositeRoundedVolume,
                    if (-oppositeRoundedVolume < BigDecimal.ZERO) -oppositeRoundedVolume else BigDecimal.ZERO
                )
                ...
                val relativeSpread =
                    if (validSpread) NumberUtils.divideWithMaxScale(absoluteSpread!!, bestAsk) else null

                val makerFees = try {
                    feeProcessor.processMakerFee(
                        limitOrder.brokerId,
                        limitOrder.accountId,
                        limitOrder.fees ?: emptyList(),
                ...
                val takerFees = try {
                    ...
                    executionContext.info(
                        "Not enough funds for fee for order id: ${order.externalId}, client: ${order.clientId}, asset: ${order.assetPairId}, volume: ${
                            NumberUtils.roundForPrint(
                                order.volume
                            )
                        }, price: ${order.takePrice()}, marketBalance: ${
                            getMarketBalance(
                                availableBalances,
                                order,
                                asset
                    ...
                    executionContext.info(
                        "Invalid fee for order id: ${order.externalId}, client: ${order.clientId}, asset: ${order.assetPairId}, volume: ${
                            NumberUtils.roundForPrint(
                                order.volume
                            )
                        }, price: ${order.takePrice()}, marketBalance: ${
                ...
                if (limitOrderCopy.reservedLimitVolume != null && limitOrderCopy.reservedLimitVolume!! > BigDecimal.ZERO) {
                    limitOrderCopy.reservedLimitVolume = NumberUtils.setScaleRoundHalfUp(
                        limitOrderCopy.reservedLimitVolume!! + if (-marketRoundedVolume < BigDecimal.ZERO) -marketRoundedVolume else -oppositeRoundedVolume,
                        limitAsset.accuracy
                    )
                }

                val newRemainingVolume = NumberUtils.setScaleRoundHalfUp(
                    limitOrderCopy.remainingVolume + marketRoundedVolume,
                    baseAsset.accuracy
                )
                ...
                if (isLimitMatched) {
                    ...
                    if (limitOrderCopy.reservedLimitVolume != null && limitOrderCopy.reservedLimitVolume!! > BigDecimal.ZERO) {
                        oppositeCashMovements.add(
                            WalletOperation(
                                limitOrder.brokerId,
                                limitOrder.accountId,
                                limitOrder.clientId,
                                if (-marketRoundedVolume < BigDecimal.ZERO) assetPair.baseAssetId else assetPair.quotingAssetId,
                                BigDecimal.ZERO,
                                -limitOrderCopy.reservedLimitVolume!!
                            )
                        )
                ...
                setMarketBalance(
                    ...
                    NumberUtils.setScaleRoundHalfUp(
                        getMarketBalance(
                            ...
                            order,
                            asset
                        ) - (if (isBuy) oppositeRoundedVolume else marketRoundedVolume).abs(), asset.accuracy
                    )
                )

                remainingVolume = if (isFullyMatched) BigDecimal.ZERO else NumberUtils.setScale(
                    remainingVolume - getVolume(
                        marketRoundedVolume.abs(),
                        order.isStraight(),
                        limitOrder.price
                    ), (if (order.isStraight()) baseAsset else quotingAsset).accuracy, order.isOrigBuySide()
                )
                ...
                val roundedAbsoluteSpread = if (absoluteSpread != null) NumberUtils.setScaleRoundHalfUp(
                    absoluteSpread,
                    assetPair.accuracy
                ) else null
                val roundedRelativeSpread = if (relativeSpread != null) NumberUtils.setScaleRoundHalfUp(
                    relativeSpread,
                    RELATIVE_SPREAD_ACCURACY
                ) else null
                ...
                marketOrderTrades.add(
                    TradeInfo(
                        ...
                        order.clientId,
                        NumberUtils.setScaleRoundHalfUp(
                            (if (isBuy) oppositeRoundedVolume else marketRoundedVolume).abs(),
                            asset.accuracy
                        ).toPlainString(),
                        asset.symbol,
                        limitOrder.clientId,
                        NumberUtils.setScaleRoundHalfUp(
                            (if (isBuy) marketRoundedVolume else oppositeRoundedVolume).abs(),
                            limitAsset.accuracy
                        ).toPlainString(),
                        limitAsset.symbol,
                        limitOrder.price,
        ...
        if (order.takePrice() == null && remainingVolume > BigDecimal.ZERO) {
            if (matchedWithZeroLatestTrade) {
                ...
                executionContext.info(
                    "No liquidity, not enough funds on limit orders, for market order id: ${order.externalId}}, client: ${order.clientId}, asset: ${order.assetPairId}, volume: ${
                        NumberUtils.roundForPrint(
                            order.volume
                        )
                    } | Unfilled: ${NumberUtils.roundForPrint(remainingVolume)}, price: ${order.takePrice()}"
                )
            }
            return MatchingResult(orderWrapper, cancelledLimitOrders)
        }
        ...
        if (order.calculateReservedVolume() > availableBalance) {
            ...
            executionContext.info(
                "Reserved volume (${order.calculateReservedVolume()}) greater than balance ($availableBalance) for order id: ${order.externalId}, client: ${order.clientId}, asset: ${order.assetPairId}, volume: ${
                    NumberUtils.roundForPrint(
                        order.volume
                    )
        ...
        val reservedBalance = if (order.calculateReservedVolume() > BigDecimal.ZERO) NumberUtils.setScale(
            order.calculateReservedVolume(),
            asset.accuracy,
            true
        ) else availableBalance
        val marketBalance = getMarketBalance(availableBalances, order, asset)
        if (marketBalance < BigDecimal.ZERO || reservedBalance < NumberUtils.setScale(
                (if (isBuy) totalLimitPrice else totalVolume),
                asset.accuracy,
                true
            )
        ) {
            order.updateStatus(OrderStatus.NotEnoughFunds, now)
            executionContext.info(
                "Not enough funds for order id: ${order.externalId}, " +
                        "client: ${order.clientId}, asset: ${order.assetPairId}, " +
                        "volume: ${NumberUtils.roundForPrint(order.volume)}, price: ${order.takePrice()}, " +
                        "marketBalance: $marketBalance : $reservedBalance < ${
                            NumberUtils.setScaleRoundUp(
                                (if (isBuy) totalLimitPrice else totalVolume),
                                asset.accuracy
                            )
                        }"
        ...
        if (order.takePrice() != null && remainingVolume > BigDecimal.ZERO) {
            val newRemainingVolume = if (order.isBuySide() || NumberUtils.equalsIgnoreScale(
                    remainingVolume,
                    BigDecimal.ZERO
                )
            ) remainingVolume else -remainingVolume
    ...
    private fun checkOrderBook(order: Order, orderBook: PriorityBlockingQueue<LimitOrder>): Boolean =
        orderBook.isEmpty() || orderBook.peek().assetPairId == order.assetPairId && orderBook.peek()
    ...
    private fun getBalance(order: Order, assetPair: AssetPair, balancesGetter: BalancesGetter): BigDecimal {
        ...
        return balancesGetter.getAvailableBalance(order.brokerId, order.accountId, order.clientId, asset)
    }

    private fun getMarketBalance(
        availableBalances: MutableMap<String, MutableMap<String, BigDecimal>>,
        order: Order,
        asset: Asset
    ): BigDecimal {
        return availableBalances.getOrPut(order.clientId) { HashMap() }[asset.symbol]!!
    }

    private fun setMarketBalance(
        availableBalances: MutableMap<String, MutableMap<String, BigDecimal>>,
        order: Order,
        asset: Asset,
        value: BigDecimal
    ) {
        availableBalances.getOrPut(order.clientId) { HashMap() }[asset.symbol] = value
    }

    private fun calculateExecutionPrice(
        order: Order,
        assetPair: AssetPair,
    ...
    private fun calculateOppositeVolume(
        order: Order,
    ...
    private fun checkExecutionPriceDeviation(
        isBuySide: Boolean,
        price: BigDecimal,
    ...
    private fun checkAndReduceBalance(
        order: LimitOrder,
        volume: BigDecimal,
        ...
        val availableBalance =
            limitBalances[order.clientId] ?: balancesGetter.getAvailableReservedBalance(
                order.brokerId,
                order.accountId,
                order.clientId,
                limitAssetId
        ...
        executionContext.info(
            "order=${order.externalId}, client=${order.clientId}, $limitAssetId : ${
                NumberUtils.roundForPrint(
                    availableBalance
                )
            } >= ${NumberUtils.roundForPrint(volume)} = $result"
        )
        if (result) {
