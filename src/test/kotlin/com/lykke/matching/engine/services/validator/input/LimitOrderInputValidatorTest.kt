...
class LimitOrderInputValidatorTest {
    ...
    fun testEmptyPrice() {
        ...
        //when
        try {
            limitOrderInputValidator.validateStopOrder(
                SingleLimitOrderParsedData(
                    getMessageWrapper(
                        singleLimitContextBuilder.build()
                    ), MIN_VOLUME_ASSET_PAIR.symbol
                )
            )
    ...
    fun testAssetDoesNotExist() {
        //given
        val singleLimitContextBuilder = getSingleLimitContextBuilder()
        singleLimitContextBuilder.limitOrder(
            getLimitOrder(
                listOf(getNewLimitFee()),
                NON_EXISTENT_ASSET_PAIR.symbol
            )
        )
        ...
        //when
        limitOrderInputValidator.validateLimitOrder(
            SingleLimitOrderParsedData(
                getMessageWrapper(
                    singleLimitContextBuilder.build()
                ), NON_EXISTENT_ASSET_PAIR.symbol
    ...
    fun testInvalidValue() {
        ...
        singleLimitContextBuilder.limitOrder(
            getLimitOrder(
                listOf(getNewLimitFee()),
                assetPair = MAX_VALUE_ASSET_PAIR.symbol,
    ...
    fun testStopOrderInvalidValue() {
        ...
        singleLimitContextBuilder.limitOrder(
            getStopOrder(
                lowerLimitPrice = BigDecimal.valueOf(8),
                lowerPrice = BigDecimal.TEN
            )
        )
        singleLimitContextBuilder.assetPair(MAX_VALUE_ASSET_PAIR)
    ...
    fun testInvalidPriceAccuracy() {
        ...
        singleLimitContextBuilder.limitOrder(
            getLimitOrder(
                listOf(getNewLimitFee()),
                assetPair = "EURUSD",
                price = BigDecimal.valueOf(0.00000000001)
            )
        )
    ...
    fun testInvalidStopOrderPricesAccuracy() {
        ...
        singleLimitContextBuilder.limitOrder(
            getLimitOrder(
                listOf(getNewLimitFee()), assetPair = "EURUSD",
    ...
    fun testInvalidVolumeAccuracy() {
        ...
        singleLimitContextBuilder.limitOrder(
            getLimitOrder(
                listOf(getNewLimitFee()),
                volume = BigDecimal.valueOf(1.000000001)
            )
        )
    ...
    fun testInvalidLimitPrice() {
        ...
        singleLimitContextBuilder.limitOrder(
            getStopOrder(
                lowerLimitPrice = BigDecimal.valueOf(9500.0), lowerPrice = BigDecimal.valueOf(9000.0),
    ...
    fun testValidLimitOrder() {
        //given
        val singleLimitContextBuilder = getSingleLimitContextBuilder()
        singleLimitContextBuilder.limitOrder(getLimitOrder(fees = listOf(getNewLimitFee())))
    ...
    fun getMessageWrapper(singleLimitContext: SingleLimitOrderContext): SingleLimitOrderMessageWrapper {
        return SingleLimitOrderMessageWrapper(
            GrpcIncomingMessages.LimitOrder.getDefaultInstance(),
            TestStreamObserver(),
            context = singleLimitContext
        )
    }
    ...
    fun getStopOrder(
        lowerLimitPrice: BigDecimal? = null, lowerPrice: BigDecimal? = null,
        upperLimitPrice: BigDecimal? = null, upperPrice: BigDecimal? = null
    ): LimitOrder {
        return LimitOrder(
            "test", "test", "BTCUSD", "", "", "test", BigDecimal.ONE,
            BigDecimal.ONE, OrderStatus.InOrderBook.name, Date(), Date(), Date(), BigDecimal.ONE, null,
            type = LimitOrderType.STOP_LIMIT, fees = listOf(getNewLimitFee()),
