...
class MultiOrderFilterTest {
    ...
    private fun sortedOrders() = listOf(
        buildLimitOrder(volume = -5.0, price = 1.1),
    ...
    private fun createFilter(isTrustedClient: Boolean) = MultiOrderFilter(
        isTrustedClient,
        ...
        Date(),
        0,
        LogManager.getLogger(MultiOrderFilterTest::class.java.name)
    )
    ...
    private fun assertFilteredResult(filter: MultiOrderFilter) {
        ...
        assertEquals(
            ...
            filter.getResult().single { it.volume.compareTo(BigDecimal.valueOf(-5.0)) == 0 }.status
        )
        assertEquals(
            OrderStatus.InOrderBook.name,
            filter.getResult().single { it.volume.compareTo(BigDecimal.valueOf(-4.0)) == 0 }.status
        )
        assertEquals(
            OrderStatus.InOrderBook.name,
            filter.getResult().single { it.volume.compareTo(BigDecimal.valueOf(-1.0)) == 0 }.status
        )
        assertEquals(
            OrderStatus.InOrderBook.name,
            filter.getResult().single { it.volume.compareTo(BigDecimal.valueOf(10.0)) == 0 }.status
        )
        assertEquals(
            OrderStatus.InOrderBook.name,
            filter.getResult().single { it.volume.compareTo(BigDecimal.valueOf(5.0)) == 0 }.status
        ...
        assertEquals(
            OrderStatus.InOrderBook.name,
            filter.getResult().single { it.volume.compareTo(BigDecimal.valueOf(2.0)) == 0 }.status
        )
        assertEquals(
        ...
        assertEquals(
            OrderStatus.NotEnoughFunds.name,
            filter.getResult().single { it.volume.compareTo(BigDecimal.valueOf(-3.0)) == 0 }.status
        )
        assertEquals(
            OrderStatus.NotEnoughFunds.name,
            filter.getResult().single { it.volume.compareTo(BigDecimal.valueOf(7.0)) == 0 }.status
        )
    }
}