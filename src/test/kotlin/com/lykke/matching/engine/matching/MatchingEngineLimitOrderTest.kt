...
class MatchingEngineLimitOrderTest : MatchingEngineTest() {
    ...
    fun testMatchLimitOrderSellWithEmptyOrderBook() {
        ...
        assertLimitOrderMatchingResult(
            matchingResult,
            remainingVolume = BigDecimal.valueOf(-100.0),
    ...
    fun testMatchLimitOrderBuyNotEnoughFunds() {
        ...
        assertLimitOrderMatchingResult(
            matchingResult,
            status = OrderStatus.ReservedVolumeGreaterThanBalance,
            marketBalance = null
        )
    }
    ...
    fun testMatchLimitOrderPriceDeviation() {
        ...
        val matchingResult =
            match(limitOrder, getOrderBook("EURUSD", true), priceDeviationThreshold = BigDecimal.valueOf(0.08))
        assertLimitOrderMatchingResult(
            matchingResult,
            status = OrderStatus.TooHighPriceDeviation,
            marketBalance = null,
            remainingVolume = BigDecimal.valueOf(-100.0)
        )
    }
    ...
    fun testMatchLimitOrderBuyOneToOne2() {
        ...
        assertLimitOrderMatchingResult(
            matchingResult, status = OrderStatus.Processing,
        ...
        assertCashMovementsEquals(
            listOf(
                WalletOperation("", "", "Client1", "EUR", BigDecimal.valueOf(89.1), BigDecimal.ZERO),
                WalletOperation("", "", "Client1", "USD", BigDecimal.valueOf(-106.03), BigDecimal.ZERO)
            ),
        ...
        assertCashMovementsEquals(
            listOf(
                WalletOperation("", "", "Client2", "EUR", BigDecimal.valueOf(-89.1), BigDecimal.valueOf(-89.1)),
                WalletOperation("", "", "Client2", "USD", BigDecimal.valueOf(106.03), BigDecimal.ZERO)
            ),
    ...
    fun testMatchLimitOrderSellOneToOne2() {
        ...
        assertCashMovementsEquals(
            listOf(
                WalletOperation("", "", "Client2", "EUR", BigDecimal.valueOf(-91.1), BigDecimal.ZERO),
                WalletOperation("", "", "Client2", "USD", BigDecimal.valueOf(110.23), BigDecimal.ZERO)
            ),
            matchingResult.ownCashMovements
        )
        ...
        assertCashMovementsEquals(
            listOf(
                ...
                WalletOperation("", "", "Client1", "USD", BigDecimal.valueOf(-110.23), BigDecimal.valueOf(-110.23)),
                WalletOperation("", "", "Client1", "USD", BigDecimal.ZERO, BigDecimal.valueOf(-0.01))
            ),
            matchingResult.oppositeCashMovements
        )
    }
    ...
    fun testTradesAfterMatching() {
        ...
        assertLimitOrderMatchingResult(
            matchingResult,
            status = OrderStatus.Matched,
            marketBalance = BigDecimal.valueOf(872.21),
            remainingVolume = BigDecimal.ZERO,
            skipSize = 0,
            cancelledSize = 0,
            lkkTradesSize = 4,
    ...
    fun testMatchLimitOrderSellFullBalance() {
        ...
        val matchingResult = match(
            buildLimitOrder(
                clientId = "Client1", assetId = "LKK1YLKK", volume = -5495.03, price = 1.0082,
                fees = buildLimitOrderFeeInstructions(
                    type = FeeType.CLIENT_FEE,
                    takerSize = 0.0009,
                    targetClientId = "Client5"
                )
            ), getOrderBook("LKK1YLKK", true)
        )
    ...
    fun testMatchLimitOrderWithZeroLatestTrade() {
        ...
        assertLimitOrderMatchingResult(
            matchingResult, status = OrderStatus.InOrderBook,
