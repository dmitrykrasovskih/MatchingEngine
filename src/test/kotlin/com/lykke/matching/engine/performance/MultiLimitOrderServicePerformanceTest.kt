...
class MultiLimitOrderServicePerformanceTest : AbstractPerformanceTest() {
    ...
    @Test
    fun testPerformance() {
        val averageOrderProcessionTime = Runner.runTests(
            REPEAT_TIMES, ::testSmallVolume,
    ...
    fun testAdd2LimitOrder(): Double {
        ...
        counter.executeAction {
            multiLimitOrderService
                .processMessage(
                    buildMultiLimitOrderWrapper(
                        pair = "EURUSD", clientId = "Client1", orders = listOf(
                            IncomingLimitOrder(100.0, 1.2),
                            IncomingLimitOrder(100.0, 1.3)
                        ),
                        cancel = false
    ...
    fun testAddAndMatchLimitOrder(): Double {
        ...
        singleLimitOrderService.processMessage(
            messageBuilder.buildLimitOrderWrapper(
                MessageBuilder.buildLimitOrder(
                    clientId = "Client2",
                    price = 1.25,
                    volume = -150.0
                )
        ...
        counter.executeAction {
            multiLimitOrderService.processMessage(
                buildMultiLimitOrderWrapper(
                    pair = "TIMEUSD",
                    clientId = "Client5",
                    orders = listOf(IncomingLimitOrder(10.0, 26.915076), IncomingLimitOrder(10.0, 26.875076)),
                    cancel = true
                )
            )
        }
    ...
    fun testMatchWithLimitOrderForAllFunds(): Double {
        ...
        primaryOrdersDatabaseAccessor.addLimitOrder(
            MessageBuilder.buildLimitOrder(
                clientId = client,
                assetId = "BTCEUR",
                price = 4722.0,
